...
class MockBackend {
  ...
  // ── Stats + assessments (S13 T13-2/T13-3) + cost dashboard (S16 T16-3) — cụm stateless
  ...
  async getCost(window: StatsWindow = '24h'): Promise<CostResponse> {
    return mockGetCost(window);
  }

  async getCostTrend(window: StatsWindow, bucket: 'hour' | 'day', groupBy: 'model' | 'role'): Promise<CostTrendResponse> {
    return mockGetCostTrend(window, bucket, groupBy);
  }
