...
| DF-B-02 | B | UX | 🟡 | Khung "🤖 LÝ DO AI" hiện JSON policy TĨNH giống nhau mọi hồ sơ — root cause CHỐT: by-design (audit chính sách áp dụng), bug chỉ ở NHÃN FE gây hiểu nhầm là lý do riêng | fixed (re-verified PROD — CHỐT) |
| DF-B-03 | B | UX | 🟡 | Tab "Trạng thái đội" hiện đếm "1 Lỗi" nhưng card không thể click/drill-down (`cursor:auto`, không có handler) — cán bộ thấy có ca lỗi nhưng không biết ca nào, lỗi gì, phải tự mò qua Workspace (mà sidebar cũng không phân biệt được ca — xem DF-B-01) | fixed (re-verified PROD — CHỐT, đúng lúc PROD có 1 ca lỗi thật) |
| DF-B-04 | B | UX | ⚪ | Control Tower / Trạng thái đội lộ thuật ngữ kỹ thuật nội bộ ra màn hình nghiệp vụ: "Cost meter... cost per-tool chưa có (SDK không tách, D-48)" — "SDK", "D-48" là mã quyết định nội bộ đội dev, cán bộ ngân hàng đọc không hiểu | fixed (re-verified PROD — CHỐT) |
...
- **Trạng thái:** fixed (re-verified :8000 + PROD e2e trọn — CHỐT).
  **Re-verify LẦN 2 đúng script architect (seed L952 cho C902) HOÀN TẤT**: dogfood-a1 (owner C902) xin giải ngân L952 (900tr) → phiếu `display:{"customer_name":"Nguyễn Văn Test","owner_id":"C902","loan_id":"L952","amount_vnd":900000000,"lane":"yellow"}` — khớp CHÍNH XÁC 100% script yêu cầu "Nguyễn Văn Test · 900.000.000 ₫ · YELLOW · L952". UI queue hiện đúng dòng đó. PASS — 2 bằng chứng độc lập (c001/L001 6 tỷ + dogfood-a1/L952) cùng xác nhậbộn B-01 hoạt động đúng trên PROD.
...
- **Trạng thái:** fixing — FE fix (portal dropdown ra `document.body` + `position:fixed`, GIỮ nguyên `overflow:hidden` topbar cho máy chiếu nhỏ) + tự verify bằng đúng phép đo `elementFromPoint` tester đặt ra.
  **Re-verify PROD HOÀN TẤT (sau deploy `dc29769`, bundle `index-DMAHaUjP.js` xác nhận qua `<script src>`)**:
  1. Login `dogfood-a1` (role customer, đúng lưu ý FE — bell chỉ hiện cho customer) → bấm 🔔 → dropdown **NHÌN THẤY RÕ**, nổi trên canvas 3D, đọc được đủ nội dung qua `document.querySelectorAll('[data-testid="notif-item"]')` → 3 item: "Yêu cầu bị từ chối (900,000,000 VND)" (L952) + "Yêu cầu bị từ chối (1,200,000,000 VND)" (L951) + "Giải ngân thành công (800,000,000 VND)" (L950) — ĐÚNG ĐỦ CẢ 3 khoản như architect yêu cầu xác nhận bell approve-side nợ cũ.
  2. **Phép đo elementFromPoint (mượn lại chính phép đo ban đầu để xác nhận dứt khoát)**: tại tâm VÀ đáy dropdown, `document.elementFromPoint()` → cả 2 đều trả về `BUTTON.notif__item` (`data-testid="notif-item"`), `dropdown.contains(hit) === true` — hoàn toàn khác trước đây (`canvas__lobby`). PASS dứt khoát theo đúng tiêu chí tự đặt ra.
  3. Esc → dropdown đóng (`querySelector` trả `null`). PASS.
  4. Click ngoài dropdown → đóng đúng. PASS.
  - Đây cũng CHÍNH THỨC giải thích finding "bell không mở được" mà tester gặp phải lúc dogfood Lô 2 (lúc đó nghi do thao tác, không kết luận được) — hoá ra là bug B-08 đã tồn tại từ đó, không phải lỗi test.
- **Trạng thái:** fixed (re-verified PROD — CHỐT, đủ 4/4 tiêu chí: nhìn thấy + elementFromPoint đúng + Esc đóng + click-ngoài đóng).
